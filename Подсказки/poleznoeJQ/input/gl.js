gl.M.cho.O.U = {
  //js:['/Primery/1/minF/m.js'],
  html: `
  <!-- <link href="https://zam.usite.pro/db/Primery/1/minF/m.css" rel="stylesheet"> -->
  <style>/* Корекция под окно: */
  #glPodskaz > *:last-child > * {/* Содержание окна */
    width: calc(100vw - 14px);/* На весь экран */
  }
  </style>
  
  <div class="ram-V0 ram-V2 gI-inf all-c1"><div>
    <b>.focus()</b> хз установим фокус(input/textare).
    <br><b>:checked</b> соответствует всем отмеченным или выбранным элементам(input).
  </div></div>

  <pre id="zCod" class="ram-n0 all-c5" data-ram-zag="HTML пример"><div><div></div><code class="v-BG0">
  &lt;script>
  $('input[checkbox]').is(':checked');//true/false
  //! $('input[checkbox]:checked')[0]; Медленее на 74%
  // ||
  $('').attr('checked', true);  //+ Установим
  $('').attr('checked', false); //† Удалим
  // ||
  $('').prop('checked', true);  //+ Установим
  $('').prop('checked', false); //† Удалим
  &lt;/script>
  </code></div></pre>

  <script>
    zCod.$();//Подсветка HTML кода
  </script>
`};